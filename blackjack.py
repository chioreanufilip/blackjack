...
#     for key in bj_strategy["soft"]:
    ...
    def get_best_move(self,player_cards, dealer_card):
        # counts the total number of cards on the table
        player_total = sum(player_cards)

        # counts the number of A cards on the table
        ...
        if len(player_cards) == 2 and player_cards[0] == player_cards[1]:
            ...
            # searches for the current pair in the dic
            if pair_value in self.bj_strategy["pairs"]:
                move = self.bj_strategy["pairs"][pair_value] # ge the best move for current cards
    ...
    def runWithInput(self):
        ...
        # deciding the next move
            ...
            if(best_move == "Double"):
                self.Double(player_cards, best_move)
            elif (best_move == "Hit"):
                self.Hit(player_cards, dealer_card, best_move)
            else: 
                self.Split()
    ...
    def Double(self, player_cards, best_move):
        new_card = int(input("Enter additional card: "))
        player_cards.append(new_card)
        self.update_count([new_card])

    def Hit(self, player_cards, dealer_card, best_move):
        while best_move == "Hit":
            new_card = int(input("Enter additional card: "))
            player_cards.append(new_card)
            self.update_count([new_card])
